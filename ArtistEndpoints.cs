...
public static class ArtistEndpoints
{
    public static void MapArtistEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/{id}", async Task<Results<Ok<Artist>, NotFound>> (int artistid, PubContext db) =>
        {
        ...
        group.MapPut("/{id}", async Task<Results<Ok, NotFound>> (int artistid, Artist artist, PubContext db) =>
        {
        ...
        group.MapDelete("/{id}", async Task<Results<Ok, NotFound>> (int artistid, PubContext db) =>
        {
            var affected = await db.Artists
                .Where(model => model.ArtistId == artistid)
                .ExecuteDeleteAsync();
